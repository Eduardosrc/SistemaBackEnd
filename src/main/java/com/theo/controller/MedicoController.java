...
public class MedicoController {
    ...
    public ResponseEntity<MedicoDto> registrar(@Valid @RequestBody MedicoCreateDto medicoCreateDto){
        return new ResponseEntity<>(service.registrar(medicoCreateDto), HttpStatus.CREATED);
    }
    ...
    @DeleteMapping("{id}")
    public ResponseEntity<MedicoDto> eliminarPorId(@PathVariable("id") Long id){
        MedicoDto medicoDto = service.eliminarPorId(id);
        if(medicoDto == null){
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        }else{
            return new ResponseEntity<>(medicoDto, HttpStatus.OK);
